...
def comprobacionCE(agente,datos):
    ...
    if any(records) and (len(records[0]) == len(columnasCE)):
        ...
        logging.info("Falta informacion en la tabla leading_db.energy_community")
        return False,datosCE

    if datosCE['name'] == "":
        logging.info("Falta el nombre de la comunidad energetica")
        return False,datosCE
    
    if datosCE['location'] == "":
        logging.info("Falta la localizacion de la comunidad energetica")
        return False,datosCE
    ...
    if datosCE['id_administrator'] == "":
        logging.info("Falta el identificador del administrador")
        return False,datosCE
...
def comprobacionUS(agente,datos):
    ...
    if any(records) and (len(records[0]) == len(columnasCE)):
        ...
        logging.info("Falta informacion en la tabla leading_db.energy_community_consumer_profile")
...
def comprobacionGen(agente,datos):
    ...
    if any(records) and (len(records[0]) == len(columnasFV)):
        ...
        logging.info("Falta informacion en la tabla leading_db.generator")
    ...
    for d in datosFV:
        for i in contenido:
            if str(d[i]) == "":
                logging.info("Fallo en la " + i + ", no esta en la tabla")
                return False,datosFV
...
def comprobacionBat(agente,datos):
    ...
    if any(records) and (len(records[0]) == len(columnasBat)):
        ...
        logging.info("Falta informacion en la tabla leading_db.storage_system")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=logging.DEBUG,
        handlers=[RotatingFileHandler(os.path.join(direc,"Mensajes_PASO0_Output.log"), maxBytes=1000000, backupCount=4)],
